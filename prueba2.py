...
# Entrada de direcciones virtuales
print("\nIngrese direcciones virtuales (en decimal, separadas por espacios):")
virtual_addrs = list(map(int, input().split()))
...
for va in virtual_addrs:
    ...
    if marco == -1:
        print(f"Fallo de página: la página {page} no está en memoria.")
...
for i, va in enumerate(virtual_addrs):
    if physical_addrs[i] is None:
        print(f"VA {va} -> Fallo de página")
    else:
        print(f"VA {va} -> PA {physical_addrs[i]}")
